    ...
    if (status.qty_H2O_Vap + rain_effective) >= SizeVap
        ...
        if (qty_H2O_C1minusVap + (rain_remain + mem_qty_H2O_Vap)) >= SizeC1minusVap
            ...
            if (status.qty_H2O_C2 + mem_qty_H2O_C1 + rain_remain) >= SizeC2
                status.qty_H2O_C2 = SizeC2
                rain_remain = rain_effective - SizeC1 - SizeC2
            else
                qty_H2O_C2 += mem_qty_H2O_C1 + rain_remain
                rain_remain = 0
            end
        else
...
function compute_fraction!(status)
    ...
    FractionC = status.qty_H2O_C / SizeC
    status.ftsw = status.qty_H2O_C / SizeC
end