...
function leaf(unique_mtg_id, index, rank, rachis_fresh_biomass, rachis_final_length, leaf_node=Node(NodeMTG("+", "Leaf", index, 4)), parameters=default_parameters(); rng=Random.MersenneTwister(1234))
    leaf_node.rank = rank
