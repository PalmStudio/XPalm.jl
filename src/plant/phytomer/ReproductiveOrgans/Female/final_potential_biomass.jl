...
PlantSimEngine.inputs_(::FemaleFinalPotentialFruits) = (potential_number_fruits=-9999, final_potential_fruit_biomass=-Inf,)
PlantSimEngine.outputs_(::FemaleFinalPotentialFruits) = (potential_number_fruits=-9999, final_potential_fruit_biomass=-Inf,)
...
function PlantSimEngine.run!(m::FemaleFinalPotentialFruits, models, status, meteo, constants, mtg::MultiScaleTreeGraph.Node)
    ...
    status.potential_number_fruits = floor(Int, coeff_dev * m.potential_fruit_number_at_maturity)
...
function PlantSimEngine.run!(m::FemaleFinalPotentialFruits, models, status, meteo, constants, extra=nothing)
    status.potential_number_fruits = prev_value(status, :potential_number_fruits, default=0)
