...
PlantSimEngine.inputs_(::NumberFruits) = (carbon_offer_after_rm=-Inf, carbon_demand_organs=-Inf, potential_fruits_number=-Inf)
PlantSimEngine.outputs_(::NumberFruits) = (fruits_number=-Inf, carbon_demand_fruits=-Inf, carbon_offer_fruits=-Inf,)
...
function PlantSimEngine.run!(m::NumberFruits, models, status, meteo, constants, node::MultiScaleTreeGraph.Node)
    ...
    if status.TT_since_init >= m.TT_flowering && status.TT_since_init <= (m.TT_flowering + m.duration_fruit_setting)
        ...
        if status.carbon_offer_fruits == -Inf
            status.carbon_offer_fruits = 0.0
        end

        status.carbon_demand_fruits = prev_value(status, :carbon_demand_fruits, default=0.0)

        if status.carbon_demand_fruits == -Inf
            status.carbon_demand_fruits = 0.0
        end
        ...
        status.carbon_offer_fruits += plant_status_prev[:carbon_offer_after_rm]
        status.carbon_demand_fruits += plant_status_prev[:carbon_demand]
    ...
    if status.TT_since_init >= (m.TT_flowering + m.duration_fruit_setting) # At fruit setting
        ...
        trophic_status_fruits = min(1.0, status.carbon_offer_fruits / status.carbon_demand_fruits)
