...
function PlantSimEngine.run!(m::ET0_BP, models, status, meteo, constants, extra=nothing)
    ...
    Sundist = 1 + 0.033 * cos(2 * (_PI / 365) * tDay)
    Ray_extra = 24 * 60 * 0.0820 / _PI * Sundist * (SunPos * sin(Decli) * sin(m.LATITUDE) + cos(Decli) * cos(m.LATITUDE) * sin(SunPos))
    RGMax = (0.75 + 0.00002 * m.ALTITUDE) * Ray_extra
    
    day_length = 7.64 * SunPos
    PAR = 0.48 * Rg
